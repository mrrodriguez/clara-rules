...
(s/defschema BetaNode
  "These nodes exist in the beta network."
  (s/pred (comp #{ProductionNode
                  QueryNode
                  RootJoinNode
                  HashJoinNode
                  ExpressionJoinNode
...
(s/defrecord Rulebase [ ;; Map of matched type to the alpha nodes that handle them.
                       alpha-roots :- {s/Any [AlphaNode]}
...
(defn assemble
  "Assembles a session from the given components, which must be a map
   ...
   :rulebase A record matching the clara.rules.engine/Rulebase structure.
  ...
  (equals [this other]
    (let [other ^AlphaRootsWrapper other]
      (cond
        ...
        ;; NB: Cannot use `==` reliably here since it isn't know that `fact-type-hash` is non-nil in
        ;; all cases, and CLJS warns on this.
