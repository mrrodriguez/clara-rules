...
(def the-rules
  [{:doc  "Rule to determine whether it is indeed cold and windy."
    ...
    :lhs  [{:type        (if (platform/compiling-cljs?) 'clara.rules.testfacts/Temperature 'clara.rules.testfacts.Temperature)
            :constraints '[(< temperature 20)
                           (== ?t temperature)]}
           {:type        (if (platform/compiling-cljs?) 'clara.rules.testfacts/WindSpeed 'clara.rules.testfacts.WindSpeed)
   ...
   {:name   "clara.rules.test-rules-data/find-cold-and-windy-data"
    :lhs    [{:fact-binding :?fact
              :type         (if (platform/compiling-cljs?) 'clara.rules.testfacts/ColdAndWindy 'clara.rules.testfacts.ColdAndWindy)
              :constraints  []}]
    :params #{}}])
